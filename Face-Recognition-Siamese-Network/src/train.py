...
def getMiniBatch(batch_size=32, prob=0.5, path="train"):
    ...
    for _ in range(batch_size):
        ...
        if res == 0:
            ...
            while (
                len(os.listdir(os.path.join(path, p1))) < 1
                or len(os.listdir(os.path.join(path, p2))) < 1
            ):
                p1, p2 = tuple(np.random.choice(persons, size=2, replace=False))
            p1 = os.path.join(
                path, p1, random.choice(os.listdir(os.path.join(path, p1)))
            )
            p2 = os.path.join(
                path, p2, random.choice(os.listdir(os.path.join(path, p2)))
            )
            p1, p2 = np.expand_dims(cv2.imread(p1, 0), -1), np.expand_dims(
                cv2.imread(p2, 0), -1
            )
            left.append(p1)
            right.append(p2)
            ...
            p1, p2 = tuple(
                np.random.choice(
                    os.listdir(os.path.join(path, p)), size=2, replace=False
                )
            )
...
def test_oneshot(model, N, verbose=0, path="eval"):
    ...
    if verbose:
        print(
            "Got an average of {}% {} way one-shot learning accuracy".format(
                percent_correct, N
            )
        )
...
if __name__ == "__main__":
    ...
    for i in range(0, 200000):
        ...
        if i % loss_every == 0:
            ...
            print(
                "iteration {}, training loss: {:.7f}, validation loss : {:.7f}".format(
                    i, np.mean(loss_history), vloss
                )
            )
            loss_history.clear()

...
def test(path="sample/tbbt.jpg"):
    ...
    faces = [
        cv2.resize(face, (100, 100), interpolation=cv2.INTER_AREA)
        for face, _, _, _, _ in giveAllFaces(image, BGR_output=True)
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-db", "--database", dest="database", help="Saved Image Database", required=True
    )
    parser.add_argument(
        "-m", "--model", dest="model", help="Saved Model", required=True
    )
    parser.add_argument(
        "-i", "--image", nargs="+", dest="images", help="Image Paths", required=True
    )
